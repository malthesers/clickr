...
export default function AutoClicker({ autoClicker }: AutoClickerProps) {
  ...
  return (
    <div
      onClick={() => buyAutoClicker(autoClicker)}
      className={
        (autoClicker.price > clicks ? 'brightness-50 cursor-not-allowed' : 'cursor-pointer') +
        ' rounded-xl border-2 border-slate-300 py-2 px-4 duration-200 hover:bg-slate-300 hover:text-black'
      }
    >
